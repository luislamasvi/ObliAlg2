...
bool Puntero<T>::operator==(const Puntero<T>& p) const
{
	//MarcarUtilizado();
	//p.MarcarUtilizado();
	return m_Ptr == p.m_Ptr || m_Ptr != nullptr && p.m_Ptr != nullptr && Comparador<T>::Default.SonIguales(*m_Ptr, *p.m_Ptr);
}
...
bool Puntero<T>::operator<(const Puntero<T>& p) const
{
	//MarcarUtilizado();
	//p.MarcarUtilizado();
	return m_Ptr == nullptr ? p.m_Ptr != nullptr : p.m_Ptr != nullptr && Comparador<T>::Default.EsMenor(*m_Ptr, *p.m_Ptr);
}
...
bool Puntero<T>::operator>(const Puntero<T>& p) const
{
	//MarcarUtilizado();
	//p.MarcarUtilizado();
	return m_Ptr != nullptr && (p.m_Ptr == nullptr || Comparador<T>::Default.EsMayor(*m_Ptr, *p.m_Ptr));
}
